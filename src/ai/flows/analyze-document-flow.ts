...
const DelegateAssessmentSchema = z.object({
  ...
  legalConsiderations: z.string().optional().describe("Considerações legais preliminares, possíveis enquadramentos típicos (tipificação penal), ou implicações jurídicas com base nas informações disponíveis no documento."),
});
...
Siga rigorosamente as fases e instruções abaixo:
...
-   **Resumo Conciso do Documento Original**: Forneça um resumo objetivo e conciso do conteúdo principal do documento, tal como ele se apresenta. Coloque no campo 'summary'.
-   **Entidades Chave do Documento Original**: Identifique e liste as entidades chave (Pessoas, Organizações, Locais, Datas, Valores Monetários, etc.) encontradas no texto original do documento. Coloque no campo 'keyEntities'.
