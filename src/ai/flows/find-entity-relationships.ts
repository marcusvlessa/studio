...
const EntitySchema = z.object({
  ...
  properties: CustomPropertiesSchema.optional().default({}).describe('Um objeto JSON representando propriedades adicionais da entidade (ex: para Pessoa {"CPF": "XXX.XXX.XXX-XX", "RG": "12.345.678-9", "Nascimento_texto": "DD/MM/AAAA"}). Os valores dentro do JSON devem ser strings. Se não houver propriedades, envie um objeto JSON vazio como {} ou omita o campo. O campo "_internal_marker_" não deve ser usado pela IA.')
});
...
const RelationshipSchema = z.object({
  ...
  properties: CustomPropertiesSchema.optional().default({}).describe('Um objeto JSON representando propriedades adicionais do relacionamento (ex: {"data_hora": "DD/MM/AAAA HH:MM", "valor_texto": "R$ XXX,XX", "frequencia_texto": "3 vezes"}). Os valores dentro do JSON devem ser strings. Se não houver, envie {} ou omita o campo. O campo "_internal_marker_" não deve ser usado pela IA.')
});
...
2.  **Inferência e Descrição de Relacionamentos (Campo: relationships):**
    ...
    *   Para CADA relacionamento:
        ...
        *   'properties': (Opcional) Se houver propriedades, forneça um objeto JSON com detalhes (Ex: {"data_hora_inicio_texto": "DD/MM/AAAA HH:MM", "duracao_segundos_texto": "120", "valor_transferido_texto": "R$ 500,00"}). Se NÃO HOUVER, envie {} ou omita o campo 'properties'. NÃO utilize o campo "_internal_marker_" nas suas respostas.
...
const findEntityRelationshipsFlow = ai.defineFlow(
  ...
  async (input: FindEntityRelationshipsInput): Promise<FindEntityRelationshipsOutput> => {
    ...
    console.log("[findEntityRelationshipsFlow] Saída bruta da IA recebida (identifiedEntities):", JSON.stringify(rawOutput.identifiedEntities?.map(e => ({id: e.id, label: e.label, type: e.type, properties: e.properties})), null, 2));
    console.log("[findEntityRelationshipsFlow] Saída bruta da IA recebida (relationships):", JSON.stringify(rawOutput.relationships?.map(r => ({source: r.source, target: r.target, label: r.label, properties: r.properties})), null, 2));
    ...
    const parsedIdentifiedEntities: FindEntityRelationshipsOutput['identifiedEntities'] = (rawOutput.identifiedEntities || [])
      .filter(entity => entity && entity.label && entity.label.trim() && !(entity.label.startsWith("AVISO DO SISTEMA:") || entity.label.startsWith("Impossibilidade de análise")))
      .map((entity, idx) => {
        const aiProvidedId = (typeof entity.id === 'string' && entity.id.trim() !== "") ? entity.id.trim() : null;
        // Use the AI-provided ID directly if valid, otherwise generate a more descriptive fallback
        const aiOriginalId = aiProvidedId || `entidade_sem_id_ia_${entityFlowCounter++}_${(entity.label || 'sem_label').substring(0,20).replace(/[^a-zA-Z0-9_]/g, '')}`;

        if (!aiProvidedId) {
            console.warn(`[findEntityRelationshipsFlow] Entidade da IA (Label: ${entity.label}, Index: ${idx}) veio SEM ID ou com ID VAZIO! Usando ID de fallback para mapeamento: ${aiOriginalId}. O prompt da IA DEVE fornecer um 'id' válido e SIMPLES para cada 'identifiedEntities'.`);
        }
        
        // Sanitize the aiOriginalId for ReactFlow: replace non-alphanumeric (excluding underscore) with underscore, limit length.
        let baseReactFlowId = aiOriginalId.replace(/[^a-zA-Z0-9_]/g, '_').substring(0, 50);
        if (baseReactFlowId.length === 0 || /^\W+$/.test(baseReactFlowId) || baseReactFlowId.startsWith('_') || baseReactFlowId.endsWith('_')) {
            // If sanitization results in empty, only underscores, or starts/ends with underscore, generate a very generic ID.
            baseReactFlowId = `ent_gerada_${entityFlowCounter++}`;
        }
        
        // Ensure uniqueness for ReactFlow IDs if sanitization leads to collisions
        ...
        // Store the mapping from the ID the AI *should have used* (aiOriginalId) to the finalReactFlowId
        aiEntityIdToReactFlowIdMap.set(aiOriginalId, finalReactFlowId);
        if(aiProvidedId && aiProvidedId !== aiOriginalId) {
            // This case should ideally not happen if AI follows instructions to provide simple IDs
            console.warn(`[findEntityRelationshipsFlow] ID da IA ('${aiProvidedId}') difere do ID original/fallback usado para mapeamento ('${aiOriginalId}') para entidade '${entity.label}'. Mapeando ambos para '${finalReactFlowId}'.`);
            aiEntityIdToReactFlowIdMap.set(aiProvidedId, finalReactFlowId);
        }
    ...
    .map((rel, index) => {
        const originalSourceId = rel.source?.trim();
        const originalTargetId = rel.target?.trim();
        ...
        const reactFlowSourceId = aiEntityIdToReactFlowIdMap.get(originalSourceId);
        const reactFlowTargetId = aiEntityIdToReactFlowIdMap.get(originalTargetId);

        if (!reactFlowSourceId) {
            console.warn(`[findEntityRelationshipsFlow] ID de origem do relacionamento IA ('${originalSourceId}') não encontrado no mapa de IDs para o relacionamento: '${rel.label}'. Chaves do mapa: [${Array.from(aiEntityIdToReactFlowIdMap.keys()).join(', ')}]. Relacionamento ignorado.`);
            return null;
        }
        if (!reactFlowTargetId) {
            console.warn(`[findEntityRelationshipsFlow] ID de destino do relacionamento IA ('${originalTargetId}') não encontrado no mapa de IDs para o relacionamento: '${rel.label}'. Chaves do mapa: [${Array.from(aiEntityIdToReactFlowIdMap.keys()).join(', ')}]. Relacionamento ignorado.`);
            return null;
        }
        ...
        return {
            ...rel,
            id: `edge-${index}-${reactFlowSourceId}-${reactFlowTargetId}`, // Use index for more unique edge IDs if AI doesn't provide one for the edge
            source: reactFlowSourceId,
            target: reactFlowTargetId,
            properties: parsedRelProperties,
        };
    })
    .filter((edge): edge is NonNullable<typeof edge> => edge !== null) // Type guard to remove nulls
    .filter(edge => { // Final check: ensure nodes still exist in the parsedIdentifiedEntities list
